...
void Levelorder(TreeNode* root, void(*print)(Element item))
{
    Queue* queue = NULL;
    if (root == NULL)
        return;
...
int main()
{
    ...
    TreeNode* pTree[100] = { 0, }; //Tree구조 포인터배열
    ...
    //void(*print)(Element item) = Process;

    printf("\nPre-order : ");   Preorder(pTree[0]);
    printf("\n\nIn-order : ");    Inorder(pTree[0]);
    printf("\n\nPost-order : ");    Postorder(pTree[0]);
    printf("\n\nBreadth First : ");     Levelorder(pTree[0],Process);   printf("\n\n");

    DestroyQueue(pTree_Queue);   
    pTree_Queue = NULL;
    DestroyTree(pTree[0]); 
    /*
