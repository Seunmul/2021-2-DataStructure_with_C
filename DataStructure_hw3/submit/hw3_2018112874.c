...
void DoSortByIndex(Student** s, int size, int num, int* ex_sort_num_ptr)
{
    int i = 0;
    int j = 0;
...
int main()
{
    ...
    while (1)
    {
        ...
        case 1:
            ...
            char** sArr = malloc(sizeof(char**) * (cnt)); // 포인터 배열로 사용할 이중 포인터 선언
            for (int i = 0; i < cnt; i++)    // 요소 개수만큼 반복
            {
                sArr[i] = malloc(sizeof(Student));    // 각 요소에 구조체 크기만큼 메모리 할당
           ...
           // printf("%d", sizeof(char**) * cnt);
            ...
            {
                st_data = malloc(sizeof(Student**) * cnt); // 구조체 포인터 동적 할당
                for (int i = 0; i < cnt; i++)    // 요소 개수만큼 반복
                {
                    st_data[i] = malloc(sizeof(Student));    // 각 요소에 구조체 크기만큼 메모리 할당
