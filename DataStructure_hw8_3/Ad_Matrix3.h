...
void InsertVertex(GRAPH* graph, void* dataPtr)
{
    if (graphFull(graph))
        return;    //graph의 사이즈 제한만큼 넣을 수 있음

    graph->VERTEX[graph->count++] = (char*)dataPtr;
    qsort(graph->VERTEX, graph->count, sizeof(char), compareChar);
}

int findVertex(GRAPH* graph, void* dataPtr)
{
    int index = 0;
    int j = 0, k = 0;
    graph->VERTEX[index];
    ...
    {
        k = 0;
        if (graph->VERTEX[index] == (char*)dataPtr)
        {
            k = 1;
        }
        if (k == 1)
    ...
    return index; //들어온 데이터의 인덱스 번호를 반환
}

void InsertEdge(GRAPH* graph, void* fromVertex, void* toVertex,int weight)
{
...
void deleteEdge(GRAPH* graph, void* fromVertex, void* toVertex)
{
